...
class Generator(nn.Module):
    ...
    def __init__(self, batch_size, image_size=64, z_dim=100, conv_dim=64, channels = 1, n_classes = 10):
        ...
        # self.fc1 = spectral_norm(nn.Linear(self.n_classes + self.channels, conv_dim * mult * 2))
        ...
        if self.imsize == 128:
            self.l5 = nn.Sequential(spectral_norm(
                nn.ConvTranspose2d(curr_dim, int(curr_dim / 2), 4, 2, 1, bias=False),
                nn.BatchNorm2d(int(curr_dim / 2)),
                nn.ReLU(True)
            ))
    ...
    def forward(self, z, labels):
        ...
        input = torch.cat((labels_emb, z), 1) # (*, 110)

        # input = self.fc1(input) # (*, batch_size * 64 * 8) #todo
...
class Discriminator(nn.Module):
    ...
    def __init__(self, batch_size, n_classes = 10, image_size = 64, conv_dim = 64, channels = 1):
        ...
        # output layers
        self.last_adv = nn.Sequential(spectral_norm(
